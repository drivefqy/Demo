...
<div class="top">
    ...
    <a href="sum.jsp" class="boxq4 test1">营业额详情</a>
...
<table>
    <tr><th>序号</th><th>用户ID</th><th>用户账号</th><th>创建时间</th><th>上次登录时间</th><th>删除用户</th></tr>
    <%
        ...
        String sql = "";
        if (state1==null){  // 说明没输入的是ID
            sql ="select * from user1 where ID not like 'admin%' ORDER BY login desc  ";
        }
        else {
        ...
        for (int i = 0;i<pagesize;i++){
            ...
            date = resultSet.getDate("creatid");
            classify = resultSet.getTimestamp(5).toLocalDateTime().format(form);
            tmp++;
    ...
    <tr>
        ...
        <td><%=date%></td>
        <td><%=classify%></td>
        <td><a href="user_del.jsp?id=<%=id%>"style="text-decoration:none;color: skyblue;">删除此用户账号</a></td>
    </tr>
    <%
            if (!resultSet.next()) {
                break;
    ...
    <tr>
        <td colspan="6">
            <a href="user.jsp?page=0"style="text-decoration:none;font: 25px 楷体;color: skyblue;">第一页</a>
            <a href="user.jsp?page=<%=pagenow-1%>"style="text-decoration:none;font: 25px 楷体;color: skyblue;">上一页</a>
            <span style="font:28px 楷体">当前页：<%=pagenow+1%></span>
            <a href="user.jsp?page=<%=pagenow+1%>"style="text-decoration:none;font: 25px 楷体;color: skyblue;">下一页</a>
            <a href="user.jsp?page=<%=sum_page-1%>"style="text-decoration:none;font: 25px 楷体;color: skyblue;">最后一页</a>
        </td>
    </tr>
    <tr>
        <td colspan="6">
